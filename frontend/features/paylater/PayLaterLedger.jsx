...
function PayLaterLedger({ title, rows, direction, ccy, ccyCode = "USD" }) {
  ...
  >{direction === "in" ? <ArrowDownLeft size={17} color={TXN_IN_COLOR} /> : <ArrowUpRight size={17} color={TXN_OUT_COLOR} />}</span><span style={{ flex: 1, minWidth: 0 }}><span style={{ display: "block", fontSize: 13.5, fontWeight: 700, color: T.ink, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</span><span style={{ display: "block", fontSize: 11, color: T.inkFaint, marginTop: 1 }}>{t.date}</span></span><span style={{ textAlign: "right", flexShrink: 0 }}>{direction === "in" ? <><span style={{ display: "block", fontSize: 13.5, fontWeight: 800, color: TXN_IN_COLOR }}>+{fmtMoney(t.amount, ccyCode)}</span><span style={{ display: "block", fontSize: 10.5, fontWeight: 700, color: T.positive, marginTop: 1 }}>Received</span></> : <><span style={{ display: "block", fontSize: 13.5, fontWeight: 800, color: TXN_OUT_COLOR }}>−{fmtMoney(t.amount, ccyCode)}</span><span style={{ display: "block", fontSize: 10.5, fontWeight: 700, color: t.status === "paid" ? T.positive : T.negative, marginTop: 1 }}>{t.status === "paid" ? "Paid" : "Pending"}</span></>}</span></div>)}</div></div>;
}
