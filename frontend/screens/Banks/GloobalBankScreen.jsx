...
function GloobalBankScreen({
  ...
  ><span style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 0 }}><span style={{ fontSize: 11, fontWeight: 800, color: T.inkFaint, textTransform: "uppercase", letterSpacing: 0.6 }}>Your balance</span><span style={{ fontSize: 28, fontWeight: 800, color: T.ink, fontFamily: T.fontDisplay, letterSpacing: 0.2 }}>{balanceStatus === "loading" ? <BalanceLoading /> : balanceStatus === "error" ? <BalanceError onRetry={onRetryBalance} /> : balanceVisible ? `${ccy}${balance}` : "•••••••"}</span></span><button
  ...
  >Recent Transactions</span><div style={{ borderRadius: T.radiusLg, background: T.surface, boxShadow: T.shadowCard, overflow: "hidden", padding: "6px 18px 12px" }}>{rows.length === 0 ? <div style={{ padding: "18px 0 8px", textAlign: "center", fontSize: 12, color: T.inkFaint, lineHeight: 1.5 }}>
    ...
    >{received ? <BankArrowDownLeft size={14} color={TXN_IN_COLOR} /> : <BankArrowUpRight size={14} color={TXN_OUT_COLOR} />}</span><span style={{ flex: 1, minWidth: 0 }}><span style={{ display: "block", fontSize: 13, fontWeight: 700, color: T.ink, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</span><span style={{ display: "block", fontSize: 10.5, color: T.inkFaint, marginTop: 1 }}>{t.date}</span></span><span style={{ fontSize: 13, fontWeight: 800, color: received ? TXN_IN_COLOR : TXN_OUT_COLOR, flexShrink: 0 }}>{received ? "+" : "−"}{ccy}{fmt(Number(t.amount || 0), ccyCode)}</span></div>;
  })}</div></div>{
