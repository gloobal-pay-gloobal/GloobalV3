...
function CountryHoldersScreen({ onBack, countryIso, data, loading, onRetry, myGloobalId }) {
  ...
  >{
    /* One flag component, everywhere. FlagCircle wraps FlagEmoji, which is
       what the registration country picker, Send Money, receipts, Coverage
       and the Bank screen all draw through — a real flag image with the
       emoji as its fallback. Printing `{country.flag}` as text instead
  ...
  }{country ? <FlagCircle flag={country.flag} size={46} /> : <span
  ...
  >{localCcy}{" · "}1 {COIN_TICKER} = {CURRENCY_SYMBOL[data.reserveCurrency] || ""}1</span>}</span></div>{
