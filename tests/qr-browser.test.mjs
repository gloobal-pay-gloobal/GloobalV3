...
describe("the scan side of the flow, as far as it can be automated", () => {
  test("\"Upload from gallery\" does not open a picker — it reveals the camera view", async () => {
    // Recording the boundary rather than asserting a feature that is not
    // there. The control is a plain button that sets the camera-granted
    // flag; there is no file input behind it, so a code cannot currently be
    // read from an image at all. That is a real gap in the product, not in
    // this test: on a device with no working camera — or for someone sent a
    // screenshot of a code — there is no way in.
    //
    // When a picker is added, this test should be replaced by the round trip
    // it currently cannot do: generate a code as one account, save the
    // image, upload it as another, and assert the payment screen fills in.
    const { page, context } = await openPage({ account: ACCOUNTS.india });
    await login(page, ACCOUNTS.india);
    await page.getByLabel("Scanner", { exact: true }).click({ force: true });
    await page.getByRole("button", { name: /Upload from gallery/i }).waitFor({ timeout: 20000 });

    assert.equal(
      await page.locator("input[type=file]").count(),
      0,
      "if a file input has appeared, the gallery path now exists — replace this test with the round trip"
    );
    await context.close();
  });
